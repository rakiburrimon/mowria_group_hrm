...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Edit Leave Request</h1>
        <div class="btn-toolbar mb-2 mb-md-0">
            <a href="{{ route('leaves.show', $leave->id) }}" class="btn btn-secondary">
    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">
                        <i class="fas fa-calendar-edit me-2"></i>Edit Leave Request
                    </h5>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('leaves.update', $leave->id) }}" id="leaveForm">
                        @csrf
                        @method('PUT')
                                ...
                                <select name="type" id="type" class="form-select @error('type') ? 'is-invalid' : ''" required>
                                    ...
                                    @foreach($leaveTypes as $type)
                                        <option value="{{ $type->code }}" 
                                                ...
                                                {{ old('type', $leave->type) == $type->code ? 'selected' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Current leave data
    const originalDays = {{ $leave->days }};
    const originalType = '{{ $leave->type }}';
    ...
    // Check leave balance (only if type or days changed)
    function checkBalance() {
        const selectedType = typeSelect.value;
        const requestedDays = parseFloat(daysInput.value) || 0;
        
        // Only check balance if type or days changed
        if (selectedType && requestedDays > 0 && 
            (selectedType !== originalType || requestedDays !== originalDays)) {
